...
public class BenchmarkUuidsTest {
  ...
  public void concurrentGeneration() throws Exception {
    ...
    System.out.println(
        "Tiny: Time = " + timems + " so " + n / timems * 1000 + " Uuids/s");

    timems = setIntegerUuids(threads, effectiveN, n, numThreads);
    System.out.println(
        "Intg: Time = " + timems + " so " + n / timems * 1000 + " Uuids/s");
    timems = setLongUuids(threads, effectiveN, n, numThreads);
    System.out.println(
        "Long: Time = " + timems + " so " + n / timems * 1000 + " Uuids/s");
    timems = setGUIDs(threads, effectiveN, n, numThreads);
    System.out.println(
        "GUID: Time = " + timems + " so " + n / timems * 1000 + " Uuids/s");
    timems = setTinyGUIDs(threads, effectiveN, n, numThreads);
    System.out.println(
        "Tiny: Time = " + timems + " so " + n / timems * 1000 + " Uuids/s");
  }

  private long setTinyGUIDs(final Thread[] threads, final int effectiveN,
                            final int n, final int numThreads)
  ...
  private long setLongUuids(final Thread[] threads, final int effectiveN,
                            final int n, final int numThreads)
      throws InterruptedException {
  ...
  private long setIntegerUuids(final Thread[] threads, final int effectiveN,
                               final int n, final int numThreads)
      throws InterruptedException {
    ...
    for (int i = 0; i < numThreads; i++) {
      threads[i] =
          new IntegerUuidTest.Generator(n / numThreads, integerUuids, i);
      threads[i].start();
    }
    for (int i = 0; i < numThreads; i++) {
      threads[i].join();
  ...
  private long setGUIDs(final Thread[] threads, final int effectiveN,
                        final int n, final int numThreads)
      throws InterruptedException {
