...
public class JsonHandlerTest {
  ...
  public void testCreateObjectNode() {
    ...
    JsonHandler.setValue(node, "tboolean", true);
    JsonHandler.setValue(node, "bytes", "bytes".getBytes());
    JsonHandler.setValue(node, "tdouble", 2.0);
    JsonHandler.setValue(node, "tint", 3);
    JsonHandler.setValue(node, "tlong", 5L);
    JsonHandler.setValue(node, "string", "string");
    assertTrue(JsonHandler.exist(node, "tboolean"));
    assertTrue(JsonHandler.exist(node, "bytes"));
    assertTrue(JsonHandler.exist(node, "tdouble"));
    assertTrue(JsonHandler.exist(node, "tint"));
    assertTrue(JsonHandler.exist(node, "tlong"));
    assertTrue(JsonHandler.exist(node, "string"));
    assertEquals(true, JsonHandler.getValue(node, "tboolean", false));
    assertArrayEquals("bytes".getBytes(),
                      JsonHandler.getValue(node, "bytes", " ".getBytes()));
    assertEquals(2.0, JsonHandler.getValue(node, "tdouble", 1.0), 0.0);
    assertEquals(3, JsonHandler.getValue(node, "tint", 2));
    assertEquals(5L, JsonHandler.getValue(node, "tlong", 0L));
    assertEquals("string", JsonHandler.getValue(node, "string", "no"));
    assertEquals("string", JsonHandler.getString(node, "string"));
    assertEquals("string", JsonHandler.getString(node, FIELDS.string));
    assertEquals("string", JsonHandler.getValue(node, FIELDS.string, "no"));
    assertEquals("true", JsonHandler.getString(node, FIELDS.tboolean));
    assertEquals(true, JsonHandler.getValue(node, FIELDS.tboolean, false));
    assertEquals("2.0", JsonHandler.getString(node, FIELDS.tdouble));
    assertTrue(2.0 == JsonHandler.getValue(node, FIELDS.tdouble, 1.0));
    assertEquals("3", JsonHandler.getString(node, FIELDS.tint));
    assertEquals(3, JsonHandler.getValue(node, FIELDS.tint, 2));
    assertEquals("5", JsonHandler.getString(node, FIELDS.tlong));
    assertEquals(5L, JsonHandler.getValue(node, FIELDS.tlong, 0L));

    assertArrayEquals("bytes".getBytes(),
                      JsonHandler.getValue(node, FIELDS.bytes, " ".getBytes()));
    ...
    assertEquals(true, map.get("tboolean"));
    assertEquals(2.0, (Double) map.get("tdouble"), 0.0);
    assertEquals(3, (int) (Integer) map.get("tint"));
    assertEquals(5L, (int) (Integer) map.get("tlong"));
    assertEquals("string", map.get("string"));
    JsonHandler.setValue(node, FIELDS.string, "string2");
    JsonHandler.setValue(node, FIELDS.bytes, "bytes2".getBytes());
    JsonHandler.setValue(node, FIELDS.tlong, 0L);
    JsonHandler.setValue(node, FIELDS.tint, 2);
    JsonHandler.setValue(node, FIELDS.tboolean, false);
    JsonHandler.setValue(node, FIELDS.tdouble, 1.0);
    assertEquals("string2", JsonHandler.getValue(node, FIELDS.string, "no"));
    assertArrayEquals("bytes2".getBytes(),
                      JsonHandler.getValue(node, FIELDS.bytes, " ".getBytes()));
    assertTrue(JsonHandler.exist(node, FIELDS.bytes));
    assertTrue(JsonHandler.exist(node, FIELDS.bytes, FIELDS.string));
    assertEquals(false, JsonHandler.getValue(node, FIELDS.tboolean, true));
    assertTrue(1.0 == JsonHandler.getValue(node, FIELDS.tdouble, 5.0));
    assertEquals(2, JsonHandler.getValue(node, FIELDS.tint, 5));
    assertEquals(0L, JsonHandler.getValue(node, FIELDS.tlong, 1L));
  }
  ...
  public void testWithFile() throws IOException {
    ...
    Map<String, Object> map2 =
        (Map<String, Object>) JsonHandler.getFromFile(fileTmp, Map.class);
